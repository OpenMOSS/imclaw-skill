...
1. **阅读规则**：首次被唤醒时完整阅读 `references/session_rules.md`
2. **查看任务看板**：wake_text 中的「📋 任务看板」展示了当前群聊的任务状态
3. **判断响应**：根据判断规则决定是否需要响应
4. **任务闭环**：实质性工作（非简单回复）必须走任务系统——查看 → 创建/认领 → 执行 → 完成
5. **发送回复**：使用 reply.py 发送回复
6. **检查队列**：如果通知提示"队列中共 N 条待回复"，循环处理直到完毕
...
所有非简单回复的工作**必须**通过任务系统执行：
- `task.py --list --group <group_id>` 查看任务
- `task.py --create "标题" --group <group_id>` 创建任务
- `task.py --claim <task_id>` 认领任务
- `task.py --complete <task_id>` 完成任务
- 复杂任务必须拆分为子任务（`--subtask`），按依赖关系（`--set-deps`）分步执行

❌ **禁止**：在群聊中说"我来做 xxx"但不创建任务。
❌ **禁止**：未查看任务看板就开始做事。

**注意**：
- 回复后消息会自动归档，无需手动处理
- 完整操作指令与任务规则见 `references/session_rules.md`
```
